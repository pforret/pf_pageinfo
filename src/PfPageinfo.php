...
class PfPageinfo
{
    ...
    public function __construct($folder = "cache", $max_age = 300, $timeout = 30)
    {
    ...
    public function get($url, $use_cookie = false, $headers_only = false)
    {
        trace("GURL::get [$url]");
        if (! $url) {
            return "";
    ...
    public function get_link_tags($html, $url)
    {
        //trace("get_link_tags");
        $result = [];
    ...
    public function extract_text($url, $html, $max_chars = 300, $title = false)
    {
        $tm = new BFTimer;
        ...
        switch (true) {
            ...
            default:
                ...
                $count2 = strlen($html);
                if ($count1 <> $count2) {
                    trace("extract_text: input is $count2 characters (after cleanup)");
                }
                $doc = new DOMDocument();
                ...
                if (! $nodelist->length) {
                    $nodelist = $xpath->query('//*[contains(@class,\'article\')]');
                }
                if (! $nodelist->length) {
                    $nodelist = $xpath->query('//*[contains(@class,\'main\')]');
                }
                if (! $nodelist->length) {
                    $nodelist = $xpath->query('//*[contains(@class,\'content\')]');
                }
                if (! $nodelist->length) {
                    $nodelist = $xpath->query('//*[contains(@class,\'page-inner\')]');
                }
                if (count($nodelist) >= 1) {
                    $text = "";
    ...
    public function get_meta_tags($html)
    {
        ...
        foreach ($matches as $match) {
            ...
            if (! $key) {
                $key = quickmatch($line, "|name=([^']+)[\s\t]|");
            }
            if (! $key) {
                $key = quickmatch($line, "|property='([^']+)'|");
            }
            if (! $key) {
                $key = quickmatch($line, "|property=([^']+)\s|");
            }
            if (! $key) {
                $key = quickmatch($line, "|http-equiv='([^']+)'|");
            }
            if (! $key) {
                $key = quickmatch($line, "|http-equiv=([^']+)\s|");
            }
            if (! $key) {
    ...
    public function extract_image($html, $url)
    {
    ...
    private function full_url($relative, $base)
    {
        ...
        if ($this->startsWith($relative, ["http://","https://"])) {
            return $relative;
        }
        // waa actually an absolute URL
        $base_parts = parse_url($base);
        // follow http/https
        if ($this->startsWith($relative, ["://"])) {
